...
#pragma warning disable CS8600 // Converting null literal or possible null value to non-nullable type.
        ...
        public IEnumerable<MarkdownAtom> Extract(string filename)
        {
        ...
        private IEnumerable<MarkdownAtom> ProcessFile(string filename)
        {
            using (FileStream fs = new FileStream(filename, FileMode.Open, FileAccess.Read))
            {
                using (StreamReader reader = new StreamReader(fs))
                {
                    ...
                    while ((charsRead = reader.Read(buffer, 0, buffer.Length)) > 0)
                    {
                        ...
                        while (startIndex < currentSegment.Length)
                        {
                            ...
                            if (nextDelimiterIndex != -1)
                            {
                                ...
                                {
                                    yield return MarkdownAtom.FromContent(segment, atomCount, _Settings.Chunking);
                                    atomCount++;
                                }
                    ...
                    {
                        yield return MarkdownAtom.FromContent(finalSegment, atomCount, _Settings.Chunking);
                        atomCount++;
                    }
                }
            }
