namespace DocumentAtom.Documents.Pdf
{
    using System;
...
#pragma warning disable CS8600 // Converting null literal or possible null value to non-nullable type.
        ...
        private IEnumerable<Atom> ProcessFile(string filename)
        {
            ...
            using (PdfDocument document = PdfDocument.Open(filename, options))
            {
                ...
                foreach (Page page in document.GetPages())
                {
                    ...
                    {
                        DataTable dt = TabulaToDataTable(table);
                        if (dt.Rows.Count > 0 && dt.Columns.Count > 0)
                        {
                            PdfTableRegion tableRegion = new PdfTableRegion
                            {
                    ...
                    List<PdfRegion> listRegions = ExtractLists(page, regions);
                    regions.AddRange(listRegions);

                    foreach (PdfListRegion listRegion in listRegions.Cast<PdfListRegion>())
                    {
                    ...
                    List<TextBlock> orderedBlocks = blocks.OrderByDescending(b => b.BoundingBox.Top).ToList();

                    foreach (TextBlock block in orderedBlocks)
                    {
        ...
        {
            PdfRectangle blockRect = block.BoundingBox;

            foreach (PdfRegion region in regions)
            {
                if (region.Processed && OverlapExists(blockRect, region.BoundingBox))
                {
                    double intersectionArea = GetIntersectionArea(blockRect, region.BoundingBox);
                    double blockArea = (blockRect.Right - blockRect.Left) * (blockRect.Top - blockRect.Bottom);
        ...
        {
            List<Table> basicTables = new List<Table>();
            List<Table> spreadsheetTables = new List<Table>();
        ...
        private static bool IsTable(Table table)
        {
            ...
            List<int> rowLengths = table.Rows.Select(row => row.Count(cell =>
                !string.IsNullOrWhiteSpace(cell?.GetText()?.Trim()))).ToList();

            // Get the most common non-zero row length (mode)
            int mostCommonLength = rowLengths.Where(len => len > 0)
                .GroupBy(len => len)
        ...
        private static bool CheckAlignmentConsistency(Table table)
        {
            ...
            IReadOnlyList<Cell> firstNonEmptyRow = table.Rows.FirstOrDefault(row =>
                row.Any(cell => !string.IsNullOrWhiteSpace(cell?.GetText())));
            if (firstNonEmptyRow == null) return false;

            List<double> columnPositions = firstNonEmptyRow
            ...
            {
                List<double> rowPositions = row.Where(cell => cell != null)
                    .Select(cell => cell.BoundingBox.Left)
                    .ToList();
        ...
        private static DataTable TabulaToDataTable(Table table)
        {
            ...
            foreach (IReadOnlyList<Cell> row in rows)
            {
        ...
        private static List<Table> MergeTables(List<Table> basicTables, List<Table> spreadsheetTables)
        {
            ...
            List<Table> mergedTables = new List<Table>(basicTables);

            if (spreadsheetTables != null)
            {
                foreach (Table spreadsheetTable in spreadsheetTables)
                {
                    bool isDuplicate = false;

                    // Compare this spreadsheet table with each basic table
                    foreach (Table basicTable in basicTables)
                    {
        ...
        private static bool AreTablesEqual(Table table1, Table table2)
        {
            ...
            PdfRectangle bbox1 = table1.BoundingBox;
            PdfRectangle bbox2 = table2.BoundingBox;
        ...
        {
            List<PdfRegion> listRegions = new List<PdfRegion>();
            ...
            for (int i = 0; i < words.Count; i++)
            {
                ...
                {
                    List<Word> markers = new List<Word>();
                    ...
                    for (int j = i; j < words.Count; j++)
                    {
                            ...
                            Math.Abs(word.BoundingBox.Left - markerX) <= tolerance)
                        {
                            ...
                            List<Word> lineWords = GetWordsOnSameLine(words, word);
                            if (lineWords.Any())
                            {
                    ...
                    if (markers.Count >= 2)
                    {
                        PdfListRegion region = new PdfListRegion
                        {
                            Items = items,
