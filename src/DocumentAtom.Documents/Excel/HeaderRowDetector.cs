...
#pragma warning disable CS8600 // Converting null literal or possible null value to non-nullable type.
        ...
        public HeaderRowResult Process(List<Row> rows, SharedStringTable sharedStringTable, int maxRowsToAnalyze = 10)
        {
            ...
            List<Dictionary<int, string>> rowData = PrepareRowData(rows, sharedStringTable, maxRowsToAnalyze);
        ...
        private List<Dictionary<int, string>> PrepareRowData(List<Row> rows, SharedStringTable sharedStringTable, int maxRowsToAnalyze)
        {
            ...
            List<Row> rowsToAnalyze = rows.Take(Math.Min(rows.Count, maxRowsToAnalyze + 1)).ToList();

            // Initialize result data structure
            List<Dictionary<int, string>> rowData = new List<Dictionary<int, string>>();
            int maxColumnIndex = 0;

            // Process each row
            foreach (Row row in rowsToAnalyze)
            {
            ...
            foreach (Dictionary<int, string> row in rowData)
            {
        ...
        private Dictionary<string, double> EvaluatePatterns(List<Dictionary<int, string>> rowData)
        {
            ...
            ColumnHeaderResult columnHeadersResult = DetectColumnHeaderPatterns(rowData);
        ...
        {
            // Cell count pattern
            List<int> nonEmptyCounts = rowData.Select(row =>
        ...
        private bool DetectHigherTextRatio(List<Dictionary<int, string>> rowData)
        {
            ...
            for (int rowIdx = 1; rowIdx < rowData.Count; rowIdx++)
            {
                foreach (string value in rowData[rowIdx].Values)
                {
                    if (IsNumeric(value))
        ...
        private bool DetectDistinctFormat(List<Dictionary<int, string>> rowData)
        {
            ...
            foreach (int colIdx in rowData[0].Keys)
            {
        ...
        private bool DetectConsistentDataColumns(List<Dictionary<int, string>> rowData)
        {
            ...
            foreach (KeyValuePair<int, List<string>> column in columnDataTypes)
            {
            ...
            foreach (KeyValuePair<int, List<string>> kvp in columnDataTypes)
            {
                int colIdx = kvp.Key;
                List<string> types = kvp.Value;
        ...
        {
            ColumnHeaderResult result = new ColumnHeaderResult();
        ...
        private bool CheckForNamedColumnHeaders(IEnumerable<string> values)
        {
            ...
            foreach (KeyValuePair<string, List<int>> group in prefixGroups)
            {
                if (group.Value.Count >= 2)
                {
                    List<int> sortedNumbers = group.Value.OrderBy(n => n).ToList();
        ...
        private bool DetectRow1Differs(List<Dictionary<int, string>> rowData)
        {
            ...
            for (int rowIdx = 1; rowIdx < rowData.Count; rowIdx++)
            {
                foreach (KeyValuePair<int, string> kvp in rowData[rowIdx])
                {
                    int colIdx = kvp.Key;
        ...
        private bool IsPureNumericHeader(List<Dictionary<int, string>> rowData)
        {
            ...
            List<int> firstRowValues = rowData[0].Values
