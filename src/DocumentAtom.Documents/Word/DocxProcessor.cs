...
#pragma warning disable CS8603 // Possible null reference return.
        ...
        private IEnumerable<Atom> ProcessFile(string filename)
        {
            ...
            using (WordprocessingDocument doc = WordprocessingDocument.Open(filename, false))
            {
                ...
                foreach (OpenXmlElement element in body.Elements())
                {
                    ...
                    {
                        NumberingProperties numProperties = paragraph.ParagraphProperties?.NumberingProperties;
                        bool isListItem = numProperties != null;
                ...
                foreach (ImagePart imagePart in doc.MainDocumentPart.ImageParts)
                {
                    ...
                    using (MemoryStream ms = new MemoryStream())
                    {
                        ...
                        DocumentFormat.OpenXml.Drawing.Blip drawing = doc.MainDocumentPart.Document.Descendants<DocumentFormat.OpenXml.Drawing.Blip>()
        ...
        private bool IsOrderedList(WordprocessingDocument doc, NumberingProperties numProperties)
        {
            try
            {
                Int32Value numberingPartId = numProperties.NumberingId.Val;
                Int32Value abstractNumId = doc.MainDocumentPart.NumberingDefinitionsPart
                    .Numbering
                    .Elements<NumberingInstance>()
                    .First(ni => ni.NumberID == numberingPartId)
                    .AbstractNumId.Val;

                AbstractNum abstractNum = doc.MainDocumentPart.NumberingDefinitionsPart
                    .Numbering
                    .Elements<AbstractNum>()
                    .First(an => an.AbstractNumberId == abstractNumId);

                // Check first level's numFmt
                Level level = abstractNum.Elements<Level>().FirstOrDefault();
