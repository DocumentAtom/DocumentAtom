namespace DocumentAtom.McpServer.Registrations
{
    ...
    /// <summary>
    /// Registration methods for CSV operations.
    /// </summary>
    public static class CsvRegistrations
    {
        ...
        /// <summary>
        /// Registers CSV tools on HTTP server.
        /// </summary>
        ...
        public static void RegisterHttpTools(McpHttpServer server, DocumentAtomSdk sdk, Serializer serializer)
        {
            server.RegisterTool(
                "csv/process",
                "Process a CSV file and extract atoms",
                new
                {
                    type = "object",
                    properties = new
                    {
                        data = new { type = "string", description = "Base64-encoded CSV file data" },
                ...
                (args) =>
                {
                    ...
                    if (!args.Value.TryGetProperty("data", out JsonElement dataProp))
                        throw new ArgumentException("CSV data is required");

                    string base64Data = dataProp.GetString() ?? throw new ArgumentException("CSV data cannot be null");
                    byte[] csvData = Convert.FromBase64String(base64Data);
                    bool extractOcr = DocumentAtomMcpServerHelpers.GetBoolOrDefault(args.Value, "extractOcr", false);

                    List<Atom> atoms = sdk.Atom.ProcessCsv(csvData, extractOcr).GetAwaiter().GetResult();
        ...
        /// <summary>
        /// Registers CSV methods on TCP server.
        /// </summary>
        ...
        {
            server.RegisterMethod("csv/process", (args) =>
            {
                if (!args.HasValue) throw new ArgumentException("Parameters required");
                if (!args.Value.TryGetProperty("data", out JsonElement dataProp))
                    throw new ArgumentException("CSV data is required");
        ...
        /// <summary>
        /// Registers CSV methods on WebSocket server.
        /// </summary>
